...
public static class TlsFailure
{
    ...
    /// <summary>
    /// 过期**之前**的提醒。★ 这是「看得见」的那一半 —— 不需要任何一次失败,连着的时候就算得出来。
    /// 返回 null = 不该打扰用户。
    ///
    /// ★★ 为什么「不要点重新配对」这句话在**这里**、而不在 <see cref="Explain"/> 的过期那一格:
    ///   它只有在证书**还能用**的时候才是对的 —— 那时私钥还在、续签这条路还通,重配确实是白毁一个身份。
    ///   证书真过期之后,续签路由**够不着**(实测见 lan-edge selftest 甲2),那句话就变成了
    ///   「否掉唯一的出路」。⇒ 建议必须跟着相位走,不能钉在一句固定的话上。
    ///
    /// ★ RenewDue 段**故意不出声**:那段系统正在正常自愈。一个正常运转也报警的系统,
    ///   两周内就会被学会忽略,于是真出事那次也没人看(CertLifecycle.ShouldAlarm 就是这条线)。
    /// </summary>
    public static string? WarnLocalCert(ClientProfile? p, DateTimeOffset now)
    {
        if (LocalCertPhase(p, now) is not { } phase || !CertLifecycle.ShouldAlarm(phase)) return null;
        if (phase == CertPhase.Expired) return Explain(TlsFailureKind.LocalDeviceCertExpired);

        var days = CertLifecycle.DaysLeft(LocalCertNotAfter(p)!.Value, now);
        return $"本机设备证书还有 {days:0.#} 天到期,本机正在自动续签(每次连接时试一次)。"
             + "★ 不要点「重新配对」:私钥还在,续签就够了,重配会把它删掉。"
             + "★ 但这句话若连着几天不消失,说明自动续签一直没成功 —— 请在它过期【之前】处理"
             + "(过期之后续签路由就够不着了,那时只剩重新配对一条路)。";
    }
    ...
    public static string Explain(TlsFailureKind kind) => kind switch
    {
        ...
        // ★★★ 2026-08-06 更正:这一格原本写的是「本机会自动续签;…★ 不要点『重新配对』」。
        //   **两句都是错的,而且错得正好把用户锁死在原地。** 实测(lan-edge selftest 甲2,三条断言):
        //     ① 带着过期证书 POST /identity/renew/enroll —— **连一个 HTTP 状态码都拿不到**,
        //        续签要用这张证书握手,而它已经握不上了 ⇒ 「会自动续签」结构上不可能兑现;
        //     ② 带着过期证书连**匿名**的 /pair/enroll 也够不着 ——「匿名」只在真的不出示证书时成立;
        //     ③ 把证书摘掉,同一个中枢、同一秒,/pair/enroll 立刻 200 + 六个词。
        //   ⇒ 过期之后**唯一**的出路就是重新配对,而原文案恰好在劝阻它,
        //     同时许诺一件正在发生、其实永远不会发生的事 —— 用户会一直等下去。
        //   ★ D89 §1.6 那张表的判断本身没错(私钥还在时不该重配),错的是**它被钉在了过期这一格**:
        //     这个 kind 的判据就是 `notAfter <= now`,所以它**只会**在已经过期时出现。
        //     那句劝阻已移到 WarnLocalCert 的过期**之前**那一段 —— 相位对了,话才是对的。
        TlsFailureKind.LocalDeviceCertExpired =>
            "本机的设备证书已经过期 —— 这种情况【只能重新配对】(先在主机上打开配对窗口,再在这里重新配一次)。"
            + "★ 自动续签这条路已经走不通了:续签本身要用这张证书跟中枢握手,而它已经握不上"
            + "(连一个 HTTP 状态码都拿不到)—— 所以【等下去不会自愈】。"
            + "★ 重新配对会换掉本机私钥,这是代价;但此刻没有更省的路,而过期【之前】本机一直在自动续签并提示。",
        TlsFailureKind.HubIdentityChanged =>
            "连上了,但对面的证书链不到你配对时钉住的那个中枢 —— 可能是主机重铸了身份,"
            + "或者这个地址上是另一台机器。这种情况【必须重新配对】。",
        // ★ 2026-08-06:这一段原来写着「与『设备证书过期』不同:那一种**不该**重新配对」——
        //   随上面那条更正一起改掉。过期之后同样只能重新配对(续签路由够不着),
        //   两者真正的区别不在**动作**,而在**代价**:那一种还有一把有用的私钥被换掉,这一种没有。
        TlsFailureKind.LocalProfileUnusable =>
            "本机的配对材料已经用不了了 —— 这种情况【只能重新配对】。"
            + "★ 本机已经没有可用的私钥或档案了,所以重新配对不会毁掉任何还有用的东西 —— "
            + "这是它与「设备证书过期」的区别:那一种重配还要搭上一把本来有用的私钥,这一种没有可搭的。",
