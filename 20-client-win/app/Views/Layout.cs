...
public static class Layout
{
    ...
    ///   标题栏 38 + 页边距 32 + 问候(大字号+留白)~107
    ///   + 日历/待办 行 330(= CalendarView.PanelHeight + 62) + 12
    ///   + 天气行 300(WeatherStackHeight) + 12
    ///   + 项目行 至少 130 ≈ 961,再留余量给不同 DPI 的字体高度差 -> 1040。
    /// ★ 审计实测(2026-08-01,拿当时的 Release 构建真的 Show 出来量的):
    ///   1480×1000 下只要项目里有 1 个方块就溢出 47px —— 与这段注释自己写的
    ///   "一开始就不出滚动条"相反。现在看不见只是因为项目数据还没接入、方块是空态。
    /// 会被工作区与最小尺寸夹住,小屏上不会超出。
    /// </summary>
    public const double PreferredWindowWidth = 1480;
    public const double PreferredWindowHeight = 1040;
