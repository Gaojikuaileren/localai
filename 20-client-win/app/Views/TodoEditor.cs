        ...
        var form = Ui.Stack(
            ...
            Ui.Panel("更多",
                Ui.Stack(Ui.Caption("优先级"), priority,
                         ...
                         // ★★★ 2026-08-05 又改了一次 —— 这是**同一晚同一类问题的第四次**:
                         //   上一版写的是「待办只存在这台电脑上,别的设备看不到」,
                         //   那在 D86 落地(家庭待办走内网同步)之后**当场变成假话**。
                         //   ⇒ 每次做出一件事,都要回头查一遍谁说过"这件事还没有"。
                         //     笼统的说法失信得最快,所以这里**逐档说清**:
                         Ui.Caption("「家庭」= 全家的事,**会同步到其它已配对设备**(D86);"
                                    + "「个人」= 只在这台电脑上,不同步、也不进备份。"),
                         Ui.Caption("备注"), notes),
                Theme.IconName.Tasks, new Thickness(0, 0, 0, 8), compact: true),

            buttons,
            status,
            Ui.Caption("新增/修改当场生效。★「家庭」的会同步到其它已配对设备;「个人」的只在这台电脑上(也不进备份)。")
        );
