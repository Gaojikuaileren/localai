...
public sealed class ChatCenter
{
    ...
    public bool Send(string sessionId, string text, IReadOnlyList<ChatAttachment>? attachments = null)
    {
        ...
        var note = hasAtt
            ...
            // ★ V24 复核:「设置 · 中枢」**方向是对的**(配对确实并进了设置),但那个名字屏幕上没有 ——
            //   设置里的分节叫「已配对的电脑」,未配对时卡片的小标题叫「连接到家里的中枢」,
            //   没有任何一处写着「设置 · 中枢」。⇒ 只把名字换成真的,不动方向。
            : "还没有配对中枢,所以这条消息没有发给模型 —— 到「设置」最下面的「已配对的电脑」里"
              + "点「开始寻找主机」,配好后这里会给出真实回复。";
