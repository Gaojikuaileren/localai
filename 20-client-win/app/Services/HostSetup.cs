...
//
// ★ 为什么这不只是"图省事":
//   铸身份那一步,CA 私钥**绑定创建它时的完整性等级**。客户端按 D46 **恒为普通用户**
//   (Elevation.cs 用 Mandatory Label SID 判)。⇒ 由客户端拉起 `localai-identity init`,
//   铸出来的身份**必然**在正确的上下文里。
//   反过来,人自己开终端跑就有一半概率开的是提权终端 —— 那会铸出一个【普通用户的 Edge
//   永远打不开】的身份,而且不可回退(只能重置重铸,所有已配对设备全部重配)。
    ...
    public static async Task<SetupStep> EnsureIdentityAsync()
    {
        ...
        if (Elevation.IsElevated())
            return new SetupStep(name, SetupOutcome.Failed,
                "这个客户端正以【管理员】身份运行 —— 在这个上下文里铸出来的 CA,"
                + "普通用户的 Edge 永远打不开(报「密钥集不存在」),而且不可回退。请用普通方式重开客户端。");
