...
public static class Selftest
{
    ...
    public static int Run()
    {
        ...
        try
        {
            ...
            // ★★★ V21:「模型」这一项**搬进管理端**了(勾组件 = 对显存的单侧权威决定)。
            //   判词跟着**反过来**:客户端左栏里**不许**再有它。
            //   ★ 这不是把断言删掉换一句轻的 —— 反向钉守的是纪律②:
            //     副机上看不见「模型」必须是**这一页不在这个 exe 里**,
            //     而不是靠一次运行期判断把它藏起来。藏起来的那种,判断写错就通了。
            if (mwStatus is not null)
                Assert(!mwStatus.Contains("new NavItem(\"model\""),
                       "★★★ 客户端左栏里**不再**有「模型」——它整页搬进管理端(V10 §2.1)");
            // ★ V21:这一块的锚点(ModelsView / ComponentPicker)已搬进管理端,断言跟着走了(admin/SelftestMoved.cs)。
            ...
            if (svSrc is not null)
            {
                ...
                // ★ V21:记忆库空态那条同上,跟着搬。
                //   ★★ 留在客户端的是**指路**那一句:这一页现在要说清记忆库去哪了 ——
                //     一个功能没了却没人说,和它坏了长得一模一样。
                Assert(svSrc.Contains("打开管理端面板") && svSrc.Contains("记忆从不离开主机"),
                       "★★ 记忆库搬走之后这一页要**说清它去哪了**,并且写明「副机看不到记忆」"
                       + "≠「副机上的 AI 没有记忆」(V10 §6 点名必须写死的那一句)");
            }
            ...
            // ══════════════════════════════════════════════════════════════
            //  ★ V21:这三块**被误搬到管理端又送了回来**。锚点(HubClient.ScanExplain /
            //    ClientTransport.TlsFailure / ChatCenter)一个都没搬,断言本来就该在这儿。
            //    ★★ 抓到它的是**编译器**(管理端里那几个类型根本不存在),不是我复查出来的 ——
            //      这正是「按分节整块切」的老毛病,只是这次撞在自检文件上。
            // ══════════════════════════════════════════════════════════════
                    {
                        // ★ 行为测试:四种情形各说各的话,不能混
            ...
            // ══════════════════════════════════════════════════════════════════
            {
                ...
                {
                    // ══════════════════════════════════════════════════════════════════
                    //  ★★★★ V21:这一组四条**整体改写**,而且必须说清它不是被改宽的。
                    //
                    //  V19 那四条钉的是:`CacheDevices` 有声明 · 有人调 · 调在活路径上 ·
                    //  那条活路径自己也被调。判词背后那件事是「项目『文件夹所在机器』下拉
                    //  里的远程机器,只来自**真的从主机拿到过**的设备表」。
                    //
                    //  今天那件事**不成立了**:`/admin/devices` 那一族在客户端里**整组净删**
                    //  (V10 §2.4 —— 它走局域网口打 `/admin/*`,按 D37/D48 结构上永远 404,
                    //  在客户端里从来没成功过一次)。⇒ 客户端**没有**远程设备表可缓存。
                    //
                    //  ★ 留着原判词只有两种下场:恒红(类型没了),或被人顺手改成一句轻的。
                    //  ⇒ 改成钉**今天真正要守的那件事**:那个下拉必须**如实只给本机**,
                    //    而且客户端里不许再长出一份「远程机器」的来源。
                    //  ★★ 这比原来的四条**更难糊弄**:原来只要有人调 `CacheDevices` 就绿,
                    //    而现在要求客户端里**根本没有**那个概念 —— 它是一条反向全表判据。
                    // ══════════════════════════════════════════════════════════════════
                    // ★★★ 第 1 条(已踩 11 次)当场又踩一次:第一版写的是 `!hubDev.Contains(...)`,
                    //   而它撞上了**我刚写下的、解释这一族为什么被删**的那段注释 ⇒ 假红。
                    //   ⇒ 用 `CodeOnly`(去注释**与**字符串字面量)再判 —— 收紧判据,不是删断言。
                    //   ★ 绝不许的两种"修法":把断言删掉;或把注释改写成绕开断言的样子。
                    var hubDevCode = CodeOnly(hubDev);
                    Assert(!hubDevCode.Contains("CacheDevices") && !hubDevCode.Contains("KnownDevices"),
                           "★★★★ 客户端里**不再有**远程设备表(`CacheDevices` / `KnownDevices` 整组已删)—— "
                           + "它们打的是 `/admin/*`,而那组路由按 D37/D48 只在主机回环口上存在,"
                           + "从局域网口打过去**结构上永远 404**。留着 = 一段从没成功过的死代码 + "
                           + "一个永远恒空却看起来会有内容的下拉");
                    Assert(!NoComments(devView).Contains("CacheDevices"),
                           "★★ 设备页里也不许再有写入点 —— 主机侧的设备列表已经在管理端了");
                    var machineFn = Slice(projEd, "static List<(string? Key, string Label)> MachineOptions", "static TextBlock FieldLabel");
                    Assert(machineFn is not null,
                           "★ 元断言:切得到 `MachineOptions` —— 切不到的话下面那条会静默变成零断言");
                    Assert(machineFn is not null && !machineFn.Contains("KnownDevices"),
                           "★★★ 「文件夹所在机器」下拉**不再**假装会有远程机器 —— "
                           + "V19 实测它从写下那天起就只有「本机」(唯一写入点在一段零调用方的死代码里),"
                           + "而那条断言一直绿。现在它**明说**只有本机,而不是留一个空着的下拉");

                    // ★ V21:原来这里钉「消费端还连着 KnownDevices」。那条随上面一起翻面了 ——
                    //   消费端现在**明说只有本机**,而那正是上一条在钉的东西。
            ...
            // ★ V21:`ModelsView.cs` 搬进管理端了 —— 它的同款判据在 admin/Selftest 的
            //   「管理端界面文案里不许有字面 **」那一条里(范围是整个 admin/,比这份还宽)。
            foreach (var vf in new[] { "SettingsView.cs", "DevicesView.cs", "ExtensionsView.cs" })
            {
            ...
            // ================= P3c 收尾(2026-08-03 用户裁定「把 P3c 收尾」)=================
            {
                ...
                // ══════════════════════════════════════════════════════════════
                //  ★★★★ 副机侧配对入口(V39 归属搬迁)· 2026-08-15
                //
                //  下面 **13 条(9 条判据 + 4 条元断言)** 是从 **`admin/SelftestMoved.cs`**
                //  搬过来的 —— 它们钉的全是 `app/Views/DevicesView.cs`(客户端这一半)的性质,
                //  而此前它们躺在管理端那份自检里,指着一个**在管理端源码根底下根本不存在**的
                //  `Views/DevicesView.cs` ⇒ 恒 null ⇒ **一条都没跑过**,而且长期连 SKIP 都没有。
                //
                //  ★ 数目对账(免得下一个人以为漏了):原登记写的是「(a) 那 6 条」,
                //    而 ⑤ 退役、⑥ 其实是**成对的两条**(「不许再说 Edge 窗口里敲 open」+
                //    「要说展开＋添加一台新电脑」——原登记只写了正向那半)⇒ 从 (a) 来的是 **6 条**;
                //    再加「主机上说这台不是主机」那一片里改了针脚的 **3 条**(原 d1 拆成两条 · 原 d8 一条)
                //    = 9 条判据。元断言 4 条是本轮按本仓纪律新加的(切片切不到时不许静默变零断言)。
                //
                //  ★ 为什么归属要紧,而不是"在哪跑都一样":
                //    管理端只装在**主机**上。把副机侧的判据放在那儿,等于让「副机那一屏对不对」
                //    由一个副机上根本没装的程序去看 —— 它跑不了,也没人会发现它没跑。
                //
                //  ★★ 上一版那条 SKIP 写着「等 V24 并入后搬」。V24 是 `9ac3919`(2026-08-09),
                //    并进 main 是 `80b6b31` —— **前提消失了六天,SKIP 还在**。
                //    判词记下来:**一条理由已经不成立的 SKIP,和一条假的 PASS 是同一种东西。**
                //
                //  ★★★ 原清单里的第 ⑤ 条(`HostSelfCard` × `SelfPairAsync` ∋ `ChangeDialRow(`)
                //    **没有跟过来,是有意的**:`HostSelfCard` 随 V21 搬进了管理端,而
                //    `SelfPairAsync` **两边都没有**(`admin/Views/HostHubView.cs:26` 逐字:
                //    「没有跟着搬,**也没有留下**」)⇒ 那条切片在两侧都是 null,
                //    照着"它留在客户端"的说法搬过来只会在这儿**恒红**。
                //    ★ 它守的性质「主机自己那台换了 IP 也要有出路」由**上面那一条**
                //      (`dv.Contains("ChangeDialRow(p)")`)守着,而且守得更对:
                //      新架构里主机那台走的**就是普通副机这条路**,按机器分档的判据本就该退役。
                // ══════════════════════════════════════════════════════════════
                //  ★ 读不到源码时**明着跳过**,不判红也不静默(第 11 条)。
                //    ★★ 这里**没有**用 `Skip(...)`:这份自检今天还没有那个函数
                //      (它是 V36 正在加的东西),照本文件当下的写法用 `Console.WriteLine`。
                //      ⇒ V36 并入后这一处该跟着换成 `Skip(...)`,好让它进 SKIP 计数。
                if (dv is null)
                    Console.WriteLine("  SKIP  副机侧配对入口那 13 条(V39 归属搬迁)"
                                      + " —— 读不到 `Views/DevicesView.cs`(发布产物旁边没有源码);"
                                      + "这一片是结构判据,只在仓库里跑得了");
                else
                {
                    var dvBody = Body(dv);
                    // ── ① ② 副机未配对那一屏:只允许「开始寻找主机」+ 网络选择(仅多网)+ 角色检测
                    var cpc = Slice(dv, "UIElement ClientPairCard", "string? _pickedNic");
                    Assert(cpc is not null,
                           "★ 元断言:切得到副机的配对入口 `ClientPairCard` —— "
                           + "切不到的话下面两条会**静默变成零断言**");
                    ...
                    Assert(cpc is not null && cpc.Contains("nics.Count > 1") && cpc.Contains("nics.Count == 1"),
                           ...
                           + "两个分支都要钉:只钉 `> 1` 的话,「永远显示选择框」也是绿的");
                    // ── ③ 敲门协议还没落地时必须【如实降级】
                    var knock = Slice(dv, "async Task KnockAsync", "string? _pickedDial");
                    Assert(knock is not null,
                           "★ 元断言:切得到 `KnockAsync` —— 切不到的话下面那条会静默变成零断言");
                    Assert(knock is not null && knock.Contains("还没上线"),
                           "★★ 敲门协议要中枢配合、现在还没有 —— 必须【如实降级】,"
                           + "绝不假装敲门已经发出去了(那会让人在副机这边干等一个不会来的响应)");
                    // ── ④ 「开始配对」要有在途闸
                    Assert(dvBody.Contains("if (_pairing) return;"),
                           "★★ 连点两次不能发出两条 enroll —— 六词卡是共用的,后一条会盖掉前一条,"
                           + "主机弹窗上那六个词在副机屏幕上就不存在了,人没得可比");
                    // ── ⑥ 403(窗口没开)的文案要指向【现在】的开窗方式
                    Assert(dvBody.Contains("展开「＋ 添加一台新电脑」"),
                           "★ 配对窗口关着时,要告诉他**主机上现在**该做哪一步");
                    Assert(!dvBody.Contains("Edge 窗口里输入 open"),
                           "★★ 「去 Edge 窗口里敲 open」是命令行时代的说法 —— 留着会把人支到黑框里");

                    // ══════════════════════════════════════════════════════════
                    //  ★★★★ 下面两条是 V39 **改了针脚**才搬过来的 —— 原样搬会恒红,
                    //    而恒红的理由是假的。两条都属于「主机上说这台不是主机」那一片(原 13 条)。
                    // ══════════════════════════════════════════════════════════
                    //  ⑦ 原第 1 条钉的是主机页上一句「【不能】填 127.0.0.1」。
                    //    实测:那句话在 `6af0bce`(V21 搬 3100 行)那次从产品里**消失了,两侧都没留下**,
                    //    而守它的断言当时正指着恒 null 的锚点 ⇒ **一次都没红过**。
                    //  ★ 但它守的那件事今天**更要紧**,而且换了更强的形状:
                    //    填地址这件事已经整个在客户端(`ChangeDialRow`),而主机那台
                    //    `ProbeAsync` 打的是回环、**与刚填进去的地址完全无关、永远 200**
                    //    ⇒ 「填错一位」与「填对」长得一模一样(DevicesView.cs:263-278 那段注释
                    //      记着 2026-08-08 对抗式复核就是照这条路走出来的)。
                    //  ⇒ 判据从"界面上有没有那句话"换成**行为**:存完地址必须去看配对通道那一格,
                    //    并且在它不通时明说。★ 实测 `PairingChannelError` 在本份自检里此前**零命中** ——
                    //    也就是说取代它的那条路,在今天之前**没有任何断言守着**。
                    var chg = Slice(dv, "UIElement ChangeDialRow(", "UIElement ClientPairCard");
                    Assert(chg is not null,
                           "★ 元断言:切得到 `ChangeDialRow` —— 切不到的话下面三条会静默变成零断言");
                    Assert(chg is not null && chg.Contains("PairingChannelError"),
                           "★★★ 改完地址要看【配对通道】那一格,不能只看 State —— "
                           + "这台是主机时业务调用走回环网关,`ProbeAsync` 打的是 127.0.0.1、"
                           + "与刚填的地址无关 ⇒ 永远 200 ⇒ 只看 State 时「填错一位」和「填对」一模一样,"
                           + "而聊天、内网同步、90 天一次的证书续签全打向那个不存在的地址");
                    Assert(chg is not null && chg.Contains("地址存下了,但这条通道连不上"),
                           "★★ 不通的时候要**说出来** —— 存下了却不说,人拿着一个绿点走开");
                    //  ⑧ 原第 8 条钉的是「在局域网里找回它」失败时也要给一条可执行的下一步,
                    //    针脚是 `HostToolsDir()`。★ 那根针**已经过期了,而且是有意的**:
                    //    产品代码 :303-307 白纸黑字写着 V21 把判据从「旁边有没有 host 工具目录」
                    //    换成「**这台装没装管理端**」,因为起中枢那件事现在只有管理端做得到。
                    //    ⇒ 原样搬 = 钉一条已被明文撤掉的期望值(ASSERTION-PITFALLS 第 1 条那个形状)。
                    //    ⇒ 针脚改钉**今天的那条线索**,判词一个字没让。
                    var findSlice = Slice(dv, "在局域网里找回它", "find.Margin");
                    Assert(findSlice is not null,
                           "★ 元断言:切得到「在局域网里找回它」那段失败路径");
                    Assert(findSlice is not null && findSlice.Contains("AdminAppPath()"),
                           "★★ 「找回它」失败也要给一条**可执行的下一步** —— 这是已配对卡上唯一一个"
                           + "看起来能修连接的按钮,人会先点它、远早于滚到第三张卡。"
                           + "★ 今天的线索是「这台装没装管理端」(V21 起,起中枢只有管理端做得到);"
                           + "没有它的话,这条失败路径只会把人支去查路由器/网段/防火墙,"
                           + "而真正要做的可能只是把本机的中枢起起来");
                }
                ...
                // ★★ V21:V4 那段里的 `cert.admin.*` 成对断言(客户端半边)整组搬进管理端 ——
                //   `HubAdmin` 与它的 `ParseServerCert` / `ParsePending` / `ParseDevices` /
                //   `ParseAck` / `ParseWindow` 都在那边了,判据要跟着锚点走(V10 §10.2)。
                //   ★ 那一组的**元断言**(登记表里每条 cert.admin.* 都要有客户端半边)也跟着搬,
                //     而且判词里那个「客户端半边」现在指的是**管理端这个进程** ——
                //     `check_contract_pairs.py` 里那 7 条的 `client_file` 已成对改到 admin/。
                // ▲▲▲ V4 追加段到此为止 ▲▲▲
            ...
            // ══════════════════════════════════════════════════════════
            {
                ...
                // ════════════════════════════════════════════════════════
                {
                    ...
                    // ══════════════════════════════════════════════════════════════════
                    //  ★★★★ 元断言①(V21):**客户端源码里不得出现任何单侧权威动作**。
                    //
                    //  这一条**取代**了原来那四条 `MayStartStack` 断言,而且必须说清
                    //  为什么这不是把判据改宽 —— 迁移地图 §1.1 点名警告过这一格:
                    //    原判据是 `iRole < (code.IndexOf("MayStartStack") is var i && i>=0 ? i : int.MaxValue)`。
                    //    `MayStartStack` 一删,`IndexOf` 返回 -1 ⇒ 取 `int.MaxValue`
                    //    ⇒ 那条断言**永远为真、与被测代码再无关系**。它不会红、不会有人发现,
                    //    而它守的正是「起栈只能有一个入口」这件事。
                    //  ⇒ 改成**正向判据**:不是问「那个许可位挡住了没有」,
                    //    而是问「客户端里**还有没有**那个被许可的动作」。
                    //
                    //  ★★ 判据的**范围由判词决定**(第 3b 条):判词说「客户端源码里」,
                    //    判据就枚举**整个 app/ 树**,不写死文件名。
                    //  ★★ 用 `CodeOnly` 去注释**与字符串字面量**:本仓的习惯是删掉之后把原文
                    //    留在注释里说明为什么删(第 1 条已踩 11 次),照字面搜会把最负责任的
                    //    写法全判成嫌疑。而消息字符串里也会出现这些词(比如上面那句指路)。
                    //  ★★★ 元断言的元断言:必须**真的读到了源码**且文件数 ≥ 20,
                    //    否则「一个都没匹配到」会静默变成「全清白」(第 10 条同族)。
                    // ══════════════════════════════════════════════════════════════════
                    {
                        var srcs = TryReadAllSources();
                        // ★ 第 11 条:读不到源码的那一趟(出厂产物原位)必须【跳过】而不是判红。
                        if (srcs.Count > 0)
                        {
                            Assert(srcs.Count >= 20, $"★ 元断言的元断言:真的扫到了客户端源码(读到 {srcs.Count} 个文件,"
                                                + "零命中不许算清白 —— 那和全清白在终端上长得一模一样)");
                            // ★ 针**拼出来**,不写成字面量 —— 否则守卫会被它自己所在的这一行喂饱
                            //   (第 1 条第 8 次带出来的写法)。这里扫的是 app/ 而本文件也在 app/ 下。
                            var needles = new (string Needle, string What)[]
                            {
                                ("/v1/gpu/" + "intended", "提交驻留集合变更(对显存的单侧权威决定)"),
                                ("/ad" + "min/", "回环管理面路由(D37/D48:只在主机回环口上存在)"),
                                ("localai-identity" + ".exe\", \"init", "铸中枢身份(不可回退)"),
                                ("run-" + "lan ", "起 LAN Edge"),
                                ("Ensure" + "StackAsync", "起栈入口"),
                                ("May" + "StartStack", "起栈许可位(它的被许可对象已经不在客户端了)"),
                            };
                            foreach (var (needle, what) in needles)
                            {
                                var hits = srcs.Where(s => CodeOnly(s.Text).Contains(needle, StringComparison.Ordinal))
                                               .Select(s => s.Path).OrderBy(x => x).ToList();
                                Assert(hits.Count == 0,
                                    $"★★★★ 元断言①:客户端源码里不得出现【{what}】—— "
                                    + "单侧权威动作全部归管理端(V10 §2「两侧协议 ⇒ 客户端留;单侧权威 ⇒ 整块搬」)。"
                                    + (hits.Count > 0 ? " 现有:" + string.Join(" · ", hits) : ""));
                            }
                        }
                    }
                ...
                {
                    // ★★ V21:止锚点从 `static string Say(` 换成 `static void LogCrash(` ——
                    //   `Say(SetupStep)` 跟着起栈搬走了(它翻译的是起栈那三档结果)。
                    //   ★★★ 这一处必须说清:锚点没了的话 `Slice` 返回 **null**,
                    //     于是下面三条**全部判红**,而代码其实是对的 —— 那是一次假红;
                    //     反过来若判据写成 `slice is null || …` 就成了恒真。两个方向都要避开。
                    var boot = Slice(v13AppSrc, "void StartAfterBootDecision", "static void LogCrash(");
                    Assert(boot is not null,
                        "★ 元断言:切得到 `StartAfterBootDecision` —— 切不到的话下面三条"
                        + "要么全假红、要么被写成恒真,两个方向都不算数");
                    ...
                    // ══════════════════════════════════════════════════════════════
                    //  ★★★★ 这一条原来是**迁移地图 §1.1 点名的那颗雷**,原文:
                    //    `iRole < (code.IndexOf("MayStartStack") is var i && i>=0 ? i : int.MaxValue)`
                    //  `MayStartStack` 一删,`IndexOf` 返回 -1 ⇒ 取 `int.MaxValue`
                    //  ⇒ 它**永远为真、与被测代码再无关系**:不会红、没人会发现,
                    //    而它守的正是「角色要喂在起栈之前」这件事。
                    //
                    //  ⇒ 改写成今天**真正要守的那件事**:起栈搬走之后,客户端这一侧
                    //    的顺序变成「判角色 → 拉管理端 → 起流」,而**拉管理端**
                    //    (`EnsureAdminAppRunning`)就是当年那个「起栈」的位置。
                    //  ★ 它仍然承重:管理端拉起来才会去起网关,而 `Gpu`/`Lease`
                    //    一起来就要拨那个网关 —— 顺序反了仍有「路由已定、网关还没起」的窗口。
                    //  ★★ 而这一版**不会变成恒真**:锚点是一个**还在**的生产调用,
                    //    它要是被删掉,`iAdmin` 会是 -1 而断言当场红(下面第一个合取项)。
                    // ══════════════════════════════════════════════════════════════
                    var iAdmin = bootCode?.IndexOf("EnsureAdminAppRunning(", StringComparison.Ordinal) ?? -1;
                    Assert(iRole >= 0 && iAdmin > iRole && iGpu > iAdmin,
                        "★★★ 顺序必须是【判角色 → 拉管理端 → 起流】:管理端是**唯一**的起栈入口,"
                        + "而两条流一起来就要拨那个网关 —— 拉晚了会有一段「路由已定、网关还没起」的窗口"
                        + $" ⟨NoteRole@{iRole} · EnsureAdminApp@{iAdmin} · Gpu@{iGpu}⟩");
            ...
            //   而门禁跑的恰恰是 Release/win-x64 那一趟。零命中与全清白在终端上长得一模一样。
            {
                ...
                else
                {
                    ...
                    if (Directory.Exists(adminRoot))
                    {
                        ...
                        // ★★★ V21 收窄一次范围,而且要说清**为什么这不是把判据改宽**:
                        //   判词说的是「管理端的**界面文案**」。而 `Selftest*.cs` 里的 `**`
                        //   在**断言消息**里 —— 那些字走 `Console.WriteLine`,不进 WPF,
                        //   一个星号都不会被人看见。判据的范围要由**判词**决定(第 3b 条),
                        //   把自检文件算进来,量的就不是判词说的那件事了。
