...
//     两个 csproj 编同一份。
//
//  ★★★ V37(2026-08-15)**更正上面这句的后半截**。它原本写的是
//    「—— 本文件底下 `StartEdgeCmd` 仍然用它,靠 using 取」。两件事都不成立了,
//    而且第二件**在写下它的那天就不准确**:
//      ① `StartEdgeCmd` 已删(留痕见下面它原来那个位置)⇒ 本文件**不再引用 `AdminApp`**;
//      ② 但 `using LocalAI.Client.Services;` **不能跟着删** —— 那句话把 using 的存在理由
//         记在了一个**次要**消费者头上。真正离不开它的是 `HubDiscovery` / `FoundHub`
//         (下面 `DiscoverEdgeDialsAsync` 里那两处),它们同住这个命名空间
//         (`app/Services/HubDiscovery.cs`,由 `localai-admin.csproj:144` Link 进来)。
//      ⇒ 照原话「StartEdgeCmd 没了所以 using 也没用了」去删,会**当场编不过**。
// ══════════════════════════════════════════════════════════════════════════════
...
public sealed class HubAdmin
{
    ...
    ///
    /// <para>★★★ V37(2026-08-15)如实记账:**本方法今天零生产调用点** ——
    /// 而**裁定是【保留】**,理由具体如下(不是"以后也许有用";那个理由本仓不接受,
    /// 见 `Views/ModelCatalog.cs` 文件头那条同款)。</para>
    ///
    /// <para>① **来路**:V21 之前的调用点在客户端 `DevicesView.cs`;V21 把 `HubAdmin` 整块搬到管理端,
    /// 客户端那一半改走 `HubClient.RediscoverAsync` + `HubDiscovery.ScanAsync`,
    /// 而**管理端这边没有补上等价入口**。⇒ 是**搬家时漏接的一条线**,不是被裁掉的功能。</para>
    ///
    /// <para>② ★★ **【本条初稿写错了,已推翻并改窄 —— 留痕不擦】**。
    /// 初稿写的是「它守的性质别处没有,`HubDiscovery.ScanAsync` 扫局域网、本方法扫本机网卡,
    /// **输入集合不一样,替代不了**」。**那是错的**,对抗式复核用代码把它顶翻了:
    ...
    /// ⇒ 输入集合不是"不一样",是**包含**:`ScanAsync` 是超集,
    ///   本方法的输出等价于 `ScanAsync().Hits.Where(h =&gt; LocalIPv4List().Contains(h.Ip))`。
    /// ★ 而且那条产品性质**今天就有人管**:`admin/Views/HostHubView.cs:659-661` 明写主机
    ///   「走法和任何一台副机**完全一样** → 打开这台的客户端 →「设备」→ 点「开始寻找主机」」,
    ///   那条路走的正是 `ScanAsync`(`app/Views/DevicesView.cs:352` → `:408`)。</para>
    ///
    /// <para>②b **改窄之后仍然成立的那一小块**(这才是本条真正的分量):
    ...
    /// ⇒ 所以它守的不是"整条性质",而是**性质在 /16–/23 与 /31、/32 上的那一段**。
    /// ★ 另有效率/形状差别(`ScanAsync` 为此要打最多 254 个探测,还得再交集一次本机 IP),
    ///   那是**次要**理由,不该拿它当主证。</para>
    ///
    /// <para>③ **它的旧调用方是因为别的原因死的,不是因为它** —— 但**时点也要说准**:
    /// 死的是 `SelfPairAsync`(主机一键自配对)。**方法本体在 V21 就没了**
    /// (`app/Views/DevicesView.cs` 里那个 `async Task SelfPairAsync`,`3187d59` 还在、`6af0bce` 已无);
    /// **V26 整族退役的是那 8 条断言**,不是方法。初稿把两者写成一件事,这里更正。
    /// 理由仍与本方法无关:跨进程之后保不住捷径又保住纪律,而逐字六词比对不能让(D47)。
    /// ★ 如实补一句**对本裁定不利的**证据:那次搬家**指名了接替者**,
    ///   就是 ② 里说的「和副机完全一样的两步」(`HostHubView.cs:26-33`)——
    ///   也就是说,调用方不是"没人接",而是**被有意换成了另一条路**。</para>
    ///
    /// <para>④ **六处仍在描述它的行为**(初稿写"五处"却列了六个,且漏了一个,已更正):
    /// `STATE.md:51` / `:223` · `DECISIONS.md:5282` / `:5742` ·
    /// **`PROJECT_PLAN_v3.0.md:2496`**(初稿漏掉的那一处) · 本仓 `HostProvision.cs:227` / `:473`。
    /// 它们把「逐张网卡找 8443、**明确跳过回环**」当作 D107 自配对链上**活着的第二道闸**在推理。
    /// 代码删了而这些描述还在,比留着这个函数更糟:下一个人会照着描述去找一个不存在的东西。</para>
    ///
    /// <para>★★ **推翻 ② 之后,为什么裁定仍然是【保留】**(说清楚,免得看着像护短):
    /// 撑住它的已经不是 ②,而是 ②b(掩码两端那一段真空)+ ④(六处描述还在,而中央三文档
    /// 不在本车道权限内、改不了)+ ① ③(搬家漏接、旧调用方死于无关原因)。
    /// ★ 这是一条**弱得多**的保留理由 —— 如实说:若将来有人补上 ②b 那段真空
    /// (例如让 `ScanAsync` 对掩码两端回落到逐网卡探测),本方法就**该删**。</para>
    ///
    /// <para>★★ **落点与删除条件**(照本仓规矩,保留必须写清这两样):
    /// **落点** = 管理端「主机中枢」页补一个「本机的拨号地址」入口(全部候选摆出来让人挑,
    /// 正是本方法返回 `List` 而不是单值的理由)。
    /// ★ 如实说明:这个落点**不一定非用本方法**——`HubDiscovery.cs` 已由
    /// `localai-admin.csproj:144` Link 进管理端,用 `ScanAsync` + 交集 `LocalIPv4List()` 也做得出来
    /// (只是要多打最多 254 个探测,且掩码两端仍然是空的,见 ②b)。
    /// **删除条件**(满足**任一**即删)= ① 用户裁定「主机那台根本不需要自己的拨号地址」;
    /// 或 ② 有人把 ②b 那段掩码真空补上(`ScanAsync` 在 `&lt; /24` 与 `/31`、`/32` 上回落到逐网卡探测)。
    /// ★★ 删的时候**必须连同上面 ④ 那【六处】描述一起处理** —— 数字别用错的那个:
    /// 初稿写"五处",照它删会把 `PROJECT_PLAN_v3.0.md:2496` 漏在原地,
    /// 而"代码没了描述还在"正是这一段存在的理由本身。</para>
    ///
    /// <para>★★★ 另记一条**给 V36 的**:`SelftestMoved.cs:986` 那条
    /// `Assert(haSrc2.Contains("DiscoverEdgeDialsAsync"))` **证明不了接线** ——
    /// 它读的是本文件的源码文本,而**下面这一行声明本身就把它喂绿了**。
    /// 本方法零调用点这件事,那条断言从头到尾没看见过。</para>
    /// </summary>
    ...
    //   ★ 本文件**不再**引用它们(V37 删掉 `StartEdgeCmd` 之后);
    //     `using LocalAI.Client.Services;` 仍然留着,是为了 `HubDiscovery` / `FoundHub`(见文件头)。

    // ════════════════════════════════════════════════════════════════════════
    //  ★★★ `StartEdgeCmd()` 已删(V37 · 2026-08-15)—— **零调用点**。
    //
    //  ★ 用词要准(初稿写错过,对抗式复核抓出来的,留痕在此):是**零调用点**,
    //    **不是**"全仓零引用"。删之前还有 4 处**非声明引用**,而且有两处是**受力的**:
    //    本文件头那句(它是 `using` 的理由)与 `localai-admin.csproj` 里 Link `AdminApp.cs` 的理由
    //    —— 两处都必须跟着改,本次都改了。写成"零引用"会让人以为删了不用管别处。
    //
    //  ★ 它做的事:在 `dist/host/` 里找 `启动Edge.cmd`,存在就返回路径,
    //    好让界面把「去双击哪个文件」直接说出来。
    //
    //  ★★ 删它**没有取消任何还在生效的产品性质**,但**次数别记错**:
    //    那条性质是被**裁掉过一次**(V22),不是两次 ——
    //      · **V21 是【搬家】,不是撤掉**。它把这个能力**交给了管理端并保持存活**:
    //        V21 当时**新增了两个活调用点**(`admin/Views/HostHubView.cs:302` 与 `:394`,
    //        `var cmd = HubAdmin.StartEdgeCmd();`),而客户端 `DevicesView.cs:298` 那次也还在调。
    //        当时的判词写得很清楚:「★ 拿不到 IP 时才退回 .cmd(总比什么都不做强)」。
    //      · **V22(`fc1fbaa`)才是真正撤掉的那一次**:它把上面两个调用点一起删掉,
    //        并如实交代行为变化「`启动Edge.cmd` 那条退路**没有保留**」
    //        (留痕在 `HostHubView.cs:472` 与 `:604`;`DECISIONS.md:5999` ·
    //        `decision-packets/admin-stack-lifecycle-2026-08-09.md`)。
    //    ⇒ 于是它从 V22(2026-08-09)起变成零调用点,到今天**六天** —— 不是陈年积尘。
    //
    //  ★★ 还有一条也别记错:**它曾经被自检钉过**。
    //    `app/Selftest.cs:4851`(提交 `29b4512`)有过
    //    `Assert(Services.HubAdmin.StartEdgeCmd() is null, "★ 没有主机端目录时启动脚本路径也返回 null,不编一个路径出来")`,
    //    后来被 `1571e60` 以**无关理由**删掉(「断言不许断言自己跑在哪个目录下」)。
    //    ⇒ 「它连自检都没有过」是错的。**"今天没有"与"从来没有"不是一回事**,
    //      而这两者的差别恰恰是判断一个东西是不是被有意放弃的关键证据。
    //
    //  ★★★ 而且今天真去指它是**有害的**,不只是没用:`dist/host/启动Edge.cmd` 里
    //    **写死着 `192.168.178.61`**(D36 明说地址要用主机名不用 IP),换路由器/换网段就废
    //    —— 见 `OPEN-ITEMS-2026-08-09.md:269`(至今「仍未了」、无人认领)。
    //    把用户指向一个写死了别人家 IP 的脚本,比不指路更糟。
    //
    //  ★ 起栈今天的**唯一**活路径:`HostProvision.EnsureStackAsync` /
    //    `EnsureEdgeAsync`(管理端「主机中枢」页),V22 已把那几条硬约束整套搬过去。
    //  ★ 不是"注释掉留着以后参考"(D84 裁定三 / ModelCatalog 那条同款):历史在 git 里。
    // ════════════════════════════════════════════════════════════════════════
