        ...
        SelfCheckCommentStrippers();
            ...
            // ---- 一键配对:按角色分流 + 配对窗口的三道闸 ----
            {
                ...
                if (hpSrc is not null && hhv is not null)
                {
                    ...
                    //
                    // ★★★★ V39 · 2026-08-15 · 这一条**加了两处**,而两处都是自证出来的,不是洁癖:
                    //   ① 上一版查的是 `hhv`(**原文,带注释**)。而「中枢自己打印的最后几行」这句话
                    //     在 `HostHubView.cs` 里出现**两处**:`:233` 是真界面文案,`:601` 是一句
                    //     **注释**(「失败时把中枢自己打印的最后几行原文摆出来 —— 全在 …」)。
                    //     ⇒ 把 `:233` 那行界面代码删掉,这条判据**照样绿**,靠 `:601` 那句注释喂着。
                    //     这正是本文件 ⑪ 那段记过的形状:**判据被注释喂绿,比没有判据更坏。**
                    //     ⇒ 改查 `Body(hhv)`(去注释)。
                    //   ② `EdgeLogTail` 原来只要求"出现过",而它的**方法声明**(`:165`)本身就满足 ——
                    //     把唯一调用点(`:231`)删掉、只留一个没人调的方法,判据也照样绿。
                    //     ⇒ 改成要求它**被调用**(`EdgeLogTail()`),那才是「日志真的被取出来摆上去」。
                    //   ★ 为什么这条要加固:V39 把「主机上说这台不是主机」那片里的同名断言
                    //     (它查的是去注释后的正文)**退役**了,理由写的是"这一条更强"。
                    //     复查时发现**当时并不更强** —— 补上这两处之后才是。
                    Assert(hhv is not null && Body(hhv).Contains("中枢自己打印的最后几行")
                           && Body(hhv).Contains("EdgeLogTail()"),
                           "★★ 起不来时,「主机中枢」那一页要把中枢自己吐的最后几行【原文】摆出来 —— "
                           + "窗口可以藏,现场不能丢。★ 判据已去注释、且要求 `EdgeLogTail()` "
                           + "**被调用**(不是只声明着)—— 两样都是为了不让这条被喂绿");
                ...
                //  ★★★★ 【V39 · 2026-08-15 更正】上面这句「**改名成**」是**假的**,而且它
                //    在这里躺了三个版本 —— 真相在下面 V23 那一段(相隔 25 行)。
                //    V21 是**拆分**不是改名:`app/Views/DevicesView.cs` 今天仍在(实测 468 行)。
                //    ⇒ 在这儿就把时态挂上,别让人读到第 322 行时先信一句假话、再走 25 行才被纠正。
                ...
                //    而且那时它们看起来像是**本轮引入的**。
                //  ★★★★ 【V39 · 2026-08-15 更正】上面那份点名单里 **`SelfPairAsync` 是错的**:
                //    它**不是**「V21 有意留在客户端」的成员 —— `HostHubView.cs:26` 逐字写着
                //    「**没有跟着搬,也没有留下**」(它跨两个进程,一个进程里做不完)。
                //    ⇒ 照这句话把它的断言"搬去客户端"会**在客户端同样恒红**,
                //      而下一个人会把那几条读成客户端欠的债。
                //    ★ 那一族 8 条已由 V26 整族退役 + 反向钉(见下面 ③ 那段与本文件 :468)。
                //  ⇒ 本轮做两件**够得着且不留假象**的事:
                ...
                if (hub is not null)
                {
                    ...
                    Assert(pendRow is not null && pendRow.Contains("p.Sas"),
                           ...
                           + "那句话就没有对象可核");
                    // ★★★★ V39 · 这一条是从下面那片「主机上说这台不是主机」里**搬上来**的
                    //   (原第 13 条,原先钉在恒 null 的 `Views/DevicesView.cs` 上 ⇒ 从没跑过)。
                    //   ★ 搬到这儿而不是在原地改指:`pendRow` 这份切片这里已经有了,
                    //     在下面再切一次等于把同一对边界字符串写两处 —— 漂了不会有任何东西红。
                    Assert(pendRow is not null && pendRow.Contains("SecondsLeft"),
                           "★ 待批准的请求要显示剩余秒数 —— 到点它在主机侧就失效了,"
                           + "界面不能装作它还在");
                    ...
                    //
                    // ──────────────────────────────────────────────────────────
                    //  ★★★★ V39 · 2026-08-15 · **这条 SKIP 到此为止**,以及它上面那段
                    //    有一句是错的。
                    //
                    //  ① **前提早就消失了**:那句「等 V24 并入后搬」——
                    //    V24 是 `9ac3919`(2026-08-09),并进 main 是 `80b6b31`。
                    //    ⇒ 前提没了 SKIP 还在,躺了六天。
                    //    ★ 判词:**一条理由已经不成立的 SKIP,和一条假的 PASS 是同一种东西** ——
                    //      两者都让人以为这里有人看着。
                    //
                    //  ② **上面那份逐条清单里,第 ⑤ 条的归属是错的**(实测,不是推理):
                    //    原断言是 `Slice(x, "UIElement HostSelfCard", "async Task SelfPairAsync")`
                    //    ∋ `ChangeDialRow(`。今天:
                    //      · `HostSelfCard` **只在管理端**(HostHubView.cs:134)—— 不在客户端;
                    //      · `SelfPairAsync` **两边都没有**(HostHubView.cs:26:没跟着搬也没留下);
                    //      · `ChangeDialRow(` 在管理端**一处代码都没有**(只剩 :21/:622 两条注释)。
                    //    ⇒ 切片在**两侧都是 null**。照这段注释说的"搬去客户端"= 在客户端**恒红**,
                    //      而红的理由是假的(它会说"主机卡上没有改地址入口",可那张卡不在客户端)。
                    //    ★ 它守的性质「主机自己那台换了 IP 也要有出路」**别处有人守**,而且守得更对:
                    //      `app/Selftest.cs` 的「换网段:改地址而不是重新配对」那一段
                    //      (`dv.Contains("ChangeDialRow(p)")`)—— 新架构里主机那台走的
                    //      **就是普通副机那条路**,所以按机器分档的那条判据本来就该退役。
                    //    ⇒ ⑤ **退役**,不搬。
                    //
                    //  ③ 其余 **①②③④⑥ 搬去 `app/Selftest.cs`**,针脚一字未改。
                    //    ★ 落地是 **6 条**不是 5:⑥ 原本就是**成对的两条**
                    //      (「不许再说『去 Edge 窗口里敲 open』」+「要说『展开＋添加一台新电脑』」),
                    //      而上面那份清单只写了正向那半 —— 一条"不许说旧的"和一条"要说新的",
                    //      少哪一半都能被改字绕过。
                    //    实测(V39 重量,不是照抄代码里的旧数):这几条的针脚在
                    //    `app/Views/DevicesView.cs` 上**全部命中**,
                    //    在 `admin/Views/HostHubView.cs` 上**一条都不命中** —— 归属方向是对的。
                    // ══════════════════════════════════════════════════════════
                    // ★ V39 处置完毕:6 条已落在 `app/Selftest.cs` 的
                    //   「副机侧配对入口(V39 归属搬迁)」那一段;第 ⑤ 条已退役(理由见上)。
                    //   这里**不再留 SKIP** —— 留着就是留一条"这儿还欠着"的假象。

                    // ══════════════════════════════════════════════════════════
                    //  ★★★★ V39 · 留下的 6 条 —— 「主机上说这台不是主机」那一片里
                    //    **该留在主机页**的那些。它们原先钉在 `Views/DevicesView.cs` 上,
                    //    而管理端源码根底下没有那个文件 ⇒ 恒 null ⇒ **一条都没跑过**。
                    //    逐条的去留理由写在下面那一大段(搜「V39 · 2026-08-15 · 这 13 条」)。
                    //  ★ 写在这儿而不是那一段旁边:`hub` 的作用域到那里已经关了,
                    //    在那边重读一遍源码会让 SRCHIT 虚高、路径写两处。
                    // ══════════════════════════════════════════════════════════
                    // ★★ 这两条钉的是 2026-08-03 那个真实事故:主机那台显示「这台不是主机」。
                    //   ★ 它们与下面那两条运行时判据(`LastProbe == NotListening` /
                    //     `LastError ∋ "没有人听"`)**不是重复**:那两条钉【分类层】不下结论,
                    //     这两条钉【界面】不许把结论印出来。分类对了而界面照旧印一句
                    //     「这台不是主机」,用户看到的还是 08-03 那一屏。
                    Assert(!body4.Contains("这台不是主机"),
                           "★★ 探测失败【不等于】这台不是主机 —— 代码只观察到"
                           + "「回环管理面没应答」,不许把它塌缩成一个证明不了的结论");
                    Assert(body4.Contains("中枢没在这台机器上运行"),
                           "★★ 要说【观察到的事】:中枢没在这台机器上运行");
                    // ★ 管理面刚答过话就证明 Edge 在跑;这一步又是本机连本机、防火墙不参与。
                    //   这两件事恰好是代码刚排除掉的,不许再让人去查。
                    //   ★★ V39:与它成对的**正向**那条(原「要指向真原因:Edge 绑在了另一个地址上」)
                    //     已退役 —— 别把这条读成"孤零零的反向钉"。它今天的正向对手是
                    //     本文件 :145 那条「把中枢自己吐的最后几行原文摆出来」:
                    //     **不许支人去查我们已经排除掉的东西,要摆出中枢自己说的话。**
                    Assert(!body4.Contains("先确认 Edge 起着、防火墙放行了。"),
                    ...
                    Assert(!body4.Contains("必须用【普通用户】双击"),
                           "★★ 不许再断言「必须普通用户」—— UAC 关闭的机器上根本没有普通身份的进程,"
                           + "那句话在那里是假的");
                    // ★ 这一条查**原文**(不去注释):「这时候必须点取消」在产品里就是一句界面文案。
                    Assert(hub.Contains("逐字一致") && hub.Contains("这时候必须点取消"),
                           "★★ 六个词【不代人比对】:界面只把词摆出来并要求人确认逐字一致,不提供跳过");
                    // ★ 看【去注释后的正文】—— 解释“为什么不能这么写”的注释里就带着这个词,不脱注释会自己撞自己
                    Assert(!body4.Contains("主机还没升级"),
                           "★ 副机那条路是【结构性】走不通(D37/D48),不许再写成\"暂时还没有\" —— "
                           + "那会让人等一个不会来的版本");
            ...
            // ---- S4 · 配对审批与设备管理接【主机本地回环管理面】(D37/D48)----
            {
                ...
                // ══════════════════════════════════════════════════════════════
                // ══════════════════════════════════════════════════════════════
                //  ★★★★ V39 · 2026-08-15 · 这 13 条**逐条处置完毕**,登记换成了断言。
                //
                //  ★ 先说数:上面那句「静态试算 ⇒ 8 绿 5 红」我**没有照抄,重量了一遍**
                //    (把 13 条针脚同时拿去 `admin/Views/HostHubView.cs` 与
                //     `app/Views/DevicesView.cs` 上跑)。实测仍是 **8 绿 5 红**,
                //    红的正是原来点名的那 5 条 —— 也就是说 V24/V30b 那批文案改动
                //    **没有动到这 13 条的任何一根针**。数没变,但**结论变了**:
                //    因为判据不是"搬对位置",是「**这条性质今天还该成立吗**」。
                //
                //  ★★ 处置分四类(逐条理由见下面每条旁边):
                //    【留 6】d3 d4 d5 d10 d11 d12 —— 主机页自己的性质,改指 `hub` 即可,全绿。
                //    【搬上去 1】d13(待批准剩余秒数)—— 挪进上面 `pendRow` 那一组,免得同一对
                //      切片边界写两处。
                //    【退役 4】d2 d6 d7 d9 —— **每一条都先确认了别处有人守**,见下。
                //    【改针脚 + 搬去客户端 2】d1 d8 —— 落在 `app/Selftest.cs`,理由见下。
                //
                //  ★★★ 退役那 4 条,守它的人分别是谁(**只删不补 = 把缺陷变回盲区**):
                //    · d2 `ProbeAsync(TheApp.Hub.Profile?.HubId)`「角色由肯定证据定」
                //      ⇒ 本文件上面那条 `probe ∋ "Admin.LastProbe == AdminProbeResult.Ok"`
                //        守着,而且**更强**:它切在 `ProbeRoleAsync` 里,不是全文件找个表达式。
                //    · d9 「中枢自己打印的最后几行」
                //      ⇒ 本文件 :145 那条钉着同一件事(它还同时钉 `EdgeLogTail`)。
                //      ★★★ 但**复查时我自己这句"更强"是错的**:那条原来查的是**原文**,
                //        而这句话在 `HostHubView.cs:601` 的一句注释里也有 ⇒ 界面代码删掉它照样绿。
                //        ⇒ 已在 :145 就地加固(改查 `Body` 去注释 + 要求 `EdgeLogTail()` 被调用),
                //          **加固之后**才当得起"更强",d9 退役才成立。原委写在那一条旁边。
                //    · d6 「Edge 绑在了另一个地址上」—— **性质本身被推翻了**。
                //      那是**我们替中枢编的理由**,而本仓已经裁过同一形状(见上面那段
                //      「别用我们猜的理由去盖住它」)。今天 `HubDownCard` 给的是
                //      「起栈时它说:…」+ 中枢自己打印的最后几行 ⇒ 取代者就是 d9/:145。
                //    · d7 `LocalIPv4List()`「摆网卡给人和 Edge 窗口那行对照」
                //      ⇒ 那套**手工对照**流程随自动装机一起没了;今天摆网卡的是
                //        `BuildNicPicker`(防火墙那一步),由上面 `nicPick` 那条钉着。
                //
                //  ★★★★ 而这里有一件**比"归属"更该记下来的事**(实测,不是推理):
                //    d1「不能】填 127.0.0.1」· d6 · d7 这三句界面文案,是在
                //    **`6af0bce`(V21 搬 3100 行进管理端)那一次从产品里消失的,两侧都没留下**,
                //    而那个提交的标题自己写着「**断言还没跟过去**」。
                //    ⇒ 守它们的断言当时正指着一个恒 null 的锚点 ⇒ **一条都没红过**。
                //    ★ 判词:**断言躺下的那段时间里,产品退化是免费的。**
                //      d6/d7 我判"性质已被更好的东西取代"(上面点了名);
                //      d1 **不能这么判** —— 见下面它自己那条。
                // ══════════════════════════════════════════════════════════════
                // ★ 留下的那 6 条**没有写在这儿**:`hub`/`hhv` 的作用域在 :1047 就关掉了
                //   (编译器当场告诉我的,不是猜的)。在这里重新 `TryReadSource` 一遍会让
                //   SRCHIT 虚高、并把同一个路径写成两处 ⇒ 改成把它们并进上面那个
                //   `if (hub is not null)` 块里(搜「V39 · 留下的 6 条」),
                //   `hub` 与 `body4` 在那儿都是现成的。
            }
