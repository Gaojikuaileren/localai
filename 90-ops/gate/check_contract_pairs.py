...
════════════════════════════════════════════════════════════════════════════
 ...
 ⑤ ★★ **网关 ↔ 模型后端的鉴权约定(D?)不在本表里,而它确实是一条跨进程约定。**
    不进本表的理由就是上面第 ③ 条:它是**请求方向**的(网关发 `Authorization`,
    llama-server 收),而 D92 的措辞是「响应契约」。硬塞进 `CONTRACTS` 会把这张表
    的语义搅浑,而**空着不写会让下一个人以为它没人管** —— 两个都不行,所以写在这儿。
    它的成对断言与元断言在 **`90-ops/gate/check_backend_auth.py`**(广度:凡起模型
    后端的地方都必须带钥匙)+ **`10-core/gateway/test_backend_key.py`**(运行期正反两向)
    + **`90-ops/verify-backend-auth.ps1`**(对真后端打的反向实证)。
    ⇒ 本表的 `DEBT` 不因它 +1:它**不是**没有配对,它是配对在另一张表上,而那张表
      同样被 `run-tests.ps1` 以「文件不在 = 判红」的接线守着。
"""
...
CONTRACTS: dict[tuple[str, str, str], dict] = {
    ...
    ("gateway", "GET", "/health"): {
        ...
        "note": "★★ 2026-08-08 更正:上一版这条写着「今天它的响应体没有任何消费者,"
                "顶层键集合改成什么都不会有人红」—— **那句话是假的,第三轮审计(D6-F1)实测把它证伪了**:"
                "服务端半边 2026-07-29 就存在,而**今天真的会红** —— 响应体加一个键 ⇒ "
                "test_lan_edge_policy.py 报 9 PASS / 1 FAIL / exit 1。"
                "★ 债**仍然欠着**,理由变了:欠的不是「没人红」,是"
                "**没有一条【登记在册的消费者半边】在读它** —— 90-ops\\start-stack.ps1:144 "
                "只看 curl 退出码、**不解析响应体**,而它才是那个真正的跨进程消费者。"
                "⇒ 那条红是别处的断言顺带咬住的,不是这条契约自己的判据。"
                "★★★ **不许走 state 豁免**:D6-F2 证明 state 是**无人校验的自由文本**,"
                "任何以 state 实现的豁免**天生是后门**(把这里改成 paired 就能让 DEBT 归零,"
                "而一个字的行为都没变 —— 那正是本仓最恨的形状)。"
                "⇒ 推荐修法:让 start-stack.ps1:144 **解析响应体的顶层键集合**,"
                "DEBT 才真的归零,而且那条判据**可以为假**。"
                "★ 另记一条与本行相邻的缺陷(审计 C 级,本次未修):第⑤组「有没有消费者」"
                "按路径匹配、**不认服务名** ⇒ 本行今天被算成「有消费者」,"
                "而唯一命中的其实是 **speech** 那个客户端文件。",
    },
...
#  ★★ 2026-08-08 更正:这里原本跟着一句「而它今天的响应体**确实没有任何消费者**
#    (`start-stack.ps1` 只看 curl 退出码)」—— **说宽了**。第三轮审计 D6-F1 实测:
#    响应体加一个键 ⇒ test_lan_edge_policy.py 判红(9 PASS / 1 FAIL / exit 1)。
#    精确的说法见上面那条 ("gateway","GET","/health") 的 note:欠的**不是**「没人会红」,
#    是**没有一条登记在册的消费者半边在读它**。两者不是一回事,而混着说会让人以为债是假的。
