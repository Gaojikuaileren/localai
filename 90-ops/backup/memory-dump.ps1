...
$pgFiles = @(Get-ChildItem $Stage -File | Where-Object { $_.Name -match '^(memory_.*\.dump|globals_.*\.sql|PG_VERSION\.txt|RESTORE-REHEARSAL\.txt)$' })
Write-Host ("  PostgreSQL: {0} 个产物,共 {1:N2} MB" -f $pgFiles.Count, (($pgFiles | Measure-Object Length -Sum).Sum / 1MB))
$reh = Get-Content (Join-Path $Stage 'RESTORE-REHEARSAL.txt') -Raw -EA SilentlyContinue
if ($reh) { Write-Host ("    自证可恢复: {0}" -f $reh.Trim()) }
...
foreach ($j in $qdrantJobs) {
  ...
  foreach ($c in $colls) {
    ...
    # ★★ 自证可恢复:把刚产出的快照【当场恢复成一个临时 collection】并比对点数。
    #    一个恢复不出来的快照和没有快照是一样的 —— 不能只验「文件存在且非空」。
    $probe = "_restore_probe_$n"
    $srcCnt = (Invoke-RestMethod "http://127.0.0.1:$($j.Port)/collections/$n" -Headers $h).result.points_count
    try { Invoke-RestMethod "http://127.0.0.1:$($j.Port)/collections/$probe" -Method Delete -Headers $h -EA SilentlyContinue | Out-Null } catch {}
    $uri = 'file:///' + ($sf -replace '\\','/')
    try {
      Invoke-RestMethod "http://127.0.0.1:$($j.Port)/collections/$probe/snapshots/recover?wait=true" `
        -Method Put -Headers $h -ContentType 'application/json' -Body (@{ location = $uri } | ConvertTo-Json) -EA Stop | Out-Null
      $dstCnt = (Invoke-RestMethod "http://127.0.0.1:$($j.Port)/collections/$probe" -Headers $h).result.points_count
    } catch {
      return (Fail "$($j.Name)/$n 快照【恢复不出来】:$_")
    } finally {
      try { Invoke-RestMethod "http://127.0.0.1:$($j.Port)/collections/$probe" -Method Delete -Headers $h -EA SilentlyContinue | Out-Null } catch {}
    }
    if ($srcCnt -ne $dstCnt) { return (Fail "$($j.Name)/$n 快照恢复后点数不符:源 $srcCnt vs 恢复 $dstCnt") }
    Write-Host ("           └ 自证可恢复 ✓ 点数 {0} 一致" -f $srcCnt)
  }
}
