...
foreach ($j in $qdrantJobs) {
  ...
  foreach ($c in $colls) {
    ...
    # ★ 完整性验真:快照是 tar 归档,校验 ustar magic —— 能抓住截断/损坏,
    #   而「文件存在且非空」抓不住半截文件。
    #
    #   ★★ 为什么【不】用 API 的 snapshots/recover 来验证(2026-07-28 实测):
    #      该端点在本机 Windows 上【恒失败】(连试 3 次全败):
    #        "failed to sync file ...\tmp\...\newest_clocks.json: 拒绝访问 (os error 5)"
    #      ACL 已核实正常(ai-mem 对 tmp 有 FullControl 继承)→ 是 Qdrant 的 Windows
    #      兼容问题,不是配置问题。
    #      **但快照本身是好的** —— 启动期恢复实测跑通(恢复出的 collection
    #      维度 1024 / Cosine / green 全对),见 BACKUP-REPORT 的恢复段。
    #      故此处只做结构校验;真实恢复能力由那条已实测的启动期路径承担。
    try {
      $fs = [System.IO.File]::OpenRead($sf)
      try {
        $buf = New-Object byte[] 5
        $null = $fs.Seek(257, 'Begin')          # tar 的 ustar magic 在偏移 257
