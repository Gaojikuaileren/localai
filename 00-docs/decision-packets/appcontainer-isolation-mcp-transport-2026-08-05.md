...
**⚠ 但有一处记账缺口(建议,本包不改 STATE)**:
`EnableLUA=0` 只活在两份决议包里,**没有进 `STATE.md` 的「环境事实(变更即更新)」表**。
它已经咬过两次(身份护栏那轮、本轮),第三次还会咬 ——
建议补一行:`UAC | EnableLUA=0(彻底关闭)⇒ 管理员账户无分裂 token,一切进程 High | 2026-08-06`。
(照 MEMORY 里「同一陷阱踩满三次就写进 PITFALLS 并配护栏」的纪律,这已经是第二次。)
