# 待决 6(Agent Worker 隔离路线)+ 待决 7(MCP 端点传输)· 决议包 **DRAFT-D: none**

> **DRAFT-D: none** —— 本包**有意不取 D 号**。**待用户裁定,至今未裁** —— 本包自陈性质是「**可行性核实 + 裁定建议**」,而待决 6(Agent Worker 隔离路线)与待决 7(MCP 端点传输)**今天仍在待决事项里**。
...
#### 1.4.1 ★ 收工前复核(2026-08-06):**数量从 9 变成 5,而且我说不清是什么删掉了那 4 个**

收工时按协调层要求交接这批残留,复核了一遍,结果与上面记的**不一致**,如实更正:

**当前实际残留 = 5 个**(全路径,刚刚枚举):

```
%TEMP%\s.sock                                  (创建于 3.1 小时前)
%TEMP%\u2.sock                                 (2.3 小时前)
%TEMP%\u_usertemp.sock                         (2.3 小时前)
%TEMP%\localai-acspike\out\spike-32088.sock    (110 分钟前)
%TEMP%\localai-acspike\out\spike-32196.sock    (106 分钟前)
```

与上面那份清单对比:

- `%TEMP%` 根下那 **3 个没变**;
- `out\` 下原先记的 6 个(`spike-{16196,32368,32720,5228,8272}.sock` + `spike.sock`)**全部消失**;
- 现在 `out\` 下的 2 个是**后来新产生的**(PID 32088 / 32196,来自 `--self-exempt` 与
  `LOCALAI_TARGET_PORT` 那两轮),不在原清单里。

**★ 那 6 个是被什么删掉的,我没有观察到,也不去编。** 排掉的几种:
没有重启(`pg-mem` 等四个服务自更早就一直 Running);
勘察程序 `run` 开头那句清 out 目录的循环对 socket 一样会失败(它是 `try/catch` 吞掉的)。
⇒ 只能记成**未归因**。
...
| 上面原来的说法 | 复核后能站住的说法 |
|---|---|
| 「除重启/系统清理外**我没有办法**删掉」 | 「**我试过的五种方法**都删不掉,且已持续 ≥3.1 小时」 |
| 隐含「永久残留」 | **「永久」没有被证明** —— 有 4 个在没有重启的情况下自己不见了 |

我今天又重试了一次 `File.Delete`(5 个全试):**5 个全部仍然失败**。
所以「过一会儿就能删」也**不成立** —— 3.1 小时后照旧。两个方向都不下断言。

#### 1.4.2 交接:谁该接这批残留,以及为什么

**① 清理本身 —— 交机主 / ops 车道(低优先)。**
0 字节、在 `%TEMP%`、不被任何东西引用 ⇒ **不影响任何功能**,不值得为它动机器状态。
建议顺序:

1. **下次重启后先复核一遍**(有 4 个已经在无重启的情况下消失,重启后很可能一并清掉);
2. 若仍在且想清掉,标准手段是
   `MoveFileEx(path, NULL, MOVEFILE_DELAY_UNTIL_REBOOT)` —— 它在启动早期、
   过滤驱动加载之前删文件。★ 但它写
   `HKLM\...\Session Manager\PendingFileRenameOperations`,**是机器级状态改动**,
   所以本车道**只建议、不执行**(收工阶段不动机器状态);
3. ★ **不要**为了删它们去改 `%TEMP%` 的 ACL 或跑第三方工具 —— 代价远大于收益。

**② 那条实测事实 —— 必须交 P4 车道,它是 B′ 的硬约束。**

真正要交接的不是这 5 个文件,是它们证明的那条性质:

> **AF_UNIX 的 socket 文件在机主 `%TEMP%` 下会变成孤立 ReparsePoint,
> 五种常规删法全部无效(`Error 1920`),并且该路径此后连 `bind` 都会 `WSAEINVAL(10022)`。**

这条**直接约束 B′**(把 `llama-server` 从 TCP 挪到 `.sock`,见
[egress-b-gates-impact-2026-08-06.md](egress-b-gates-impact-2026-08-06.md) §5.3):

| B′ 要注意的 | 依据 |
|---|---|
| **落点不能随手选** —— 在 `{state}` / `{cache}\tmp` / `{code}` / Windows 自己的 Temp 下实测 bind+connect+干净删除全通;**唯独机主 `%TEMP%` 下 connect 报 10022 且留死文件** | 本包 §2.5 ① |
| **起停必须处理残留** —— 否则「第二次启动静默失败」:`bind` 报 10022,而那看起来像端口/权限问题,不像「上一次的文件还在」 | §2.5 ② + 本节 |
| **不能靠「删掉再 bind」兜底** —— 删可能失败,而失败是 `Error 1920` 这种少见错误码,一般的错误处理不会认它 | 本节实测 |
| 后端挪到 `.sock` **不等于**被 AppContainer 隔离保护 —— 容器能连宿主 AF_UNIX(实测 roundtrip-ok),闸只剩文件 ACL | §2.5 ③ + §4.2 ⑥ |

⇒ **给 P4 的一句话**:B′ 若采纳,socket 落点建议放 `{state}`(实测可用、ACL 已强),
**不要放任何 temp 目录**;并且启动逻辑要把「路径上已有一个删不掉的 socket 文件」
当成一个**具名的、要告知的失败**,不是笼统的 bind 失败。
...
| 与「服务没起」可区分 | 未测 | ~~未测~~ → ✅ **已实测(D126)**,见下 | **实测可区分**:`10060`/挂住 vs `10061` |

> ### ★ 2026-08-11 更正:B 路那格「未测(应为 401 vs 连不上,形状清楚)」**已经被实测了**
>
> **D126(方向 B:给 `llama-server` 加鉴权,2026-08-10 本机真起过一次栈)**:
> 同机直连 `18081/v1/chat/completions` ⇒ **401 Invalid API Key**;`18081/props` ⇒ **401**;
> 经网关对话 ⇒ **200**。⇒ 当时写的「应为 401」**猜对了,而且现在有实测撑着**。
>
> ★★ 但同一条实测还撞出一件**当时的推断里没有**的事,比那格的结论更要紧:
> `--api-key-file` 指向**空文件** ⇒ llama-server **照常启动、而且完全不鉴权**
> (不带 key 打 `/props` 回 200)。
> ⇒ **一个 0 字节的文件会静默地把门重新打开,而所有「启动成功」的迹象都还在。**
> **「形状清楚」只在钥匙真的有内容时成立** —— 那一格当时写不出这句话,因为它没测。
>
> ★ 原文划掉不删:**带时态的留痕不会骗人,不带时态的会。**
> ★ 本包其余各格的「未测」**一个字没动** —— 只有 B 路这一格被实测覆盖了。
...
2. **落点必须先实测**:AF_UNIX 在 `{state}` / `{cache}\tmp` / `{code}` 下实测可用,
   但在机主 `%TEMP%` 下 connect 报 `10022`,且失败会留下**删不掉**的孤立 ReparsePoint
   (五种常规删法全无效,`fsutil reparsepoint query` 报 `Error 1920`),
   把该路径后续的 `bind` 也弄坏(§2.5 ①②)。⇒ 起停必须处理残留,落点不得随手选。
   > ★ **这一条的交接材料在 §1.4.2**(给 P4 车道):含 B′ 的四条硬约束、
   > 当前残留清单(收工复核 **5 个**,原记 9 个已更正)、以及「**永久**」二字**未被证明**的说明。
   > 一句话:**socket 落点放 `{state}`,不要放任何 temp 目录**;
   > 启动逻辑要把「路径上已有一个删不掉的 socket 文件」当成**具名的、要告知的失败**,
   > 而不是笼统的 bind 失败 —— 它的错误码是 `10022`,看起来像端口/权限问题。
