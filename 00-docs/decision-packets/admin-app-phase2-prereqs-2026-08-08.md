# `D132` · V19:phase 2 迁移的六条前置**已解**,外加**第七条**(它长在契约门禁自己身上)
...
**六条全解**(B3 已由用户裁定 A:删掉),但其中两条的实情比地图写的更糟
(B2 已经坏了,不是"将会坏");
...
| B3 | `ShowApprovalDialogAsync` 是死代码 | ✅ 解 —— **用户裁定 A:删掉**(2026-08-08)· 红测 7/7 |
...
## 6. ✅ 已裁(用户 2026-08-08)—— **A:删掉**

`ShowApprovalDialogAsync`(`DevicesView.cs`,36 行)零调用方,**已删**。
连同它的两个伴生字段:`_dialogOpen`(只写不读,CS0414 一直在报)与
`_popped`(自动弹窗的去重集合,静默死着 —— 编译器对它一声不吭)。

行为**没有任何变化** —— 那条路早就停了。今天在跑的仍然是 `PendingRow` 上的
批准/拒绝按钮:六个词由 `p.Sas` 摆在屏幕上,按钮写「词一致,批准」,
确认框写「逐字核对过了,批准」,旁边就是拒绝,409 有归因。

### ★★★ 删的时候又抖出**两条**寄生在它身上的断言

删之前只知道有 3 条(六词比对那组)。真删下去,自检当场又红一条:

| 断言 | 上一版的针脚 | 为什么它是假的 |
|---|---|---|
| ⑯「409 时界面必须说话」 | `body4.Contains("rst == 409")` | `rst` 这个变量名**只存在于那个死方法里**。活路径上叫 `ast2` —— 也就是说这条断言一直在**替死代码作证**,活路径那份是"顺便也对",从来没被它测过 |
| ⑰「轮询里不许自动弹窗」 | `!poll.Contains("ShowApprovalDialogAsync")` | 方法一删,判据**永远为真** ⇒ 一条守着**安全性质**的恒真断言。比没有更坏:它让人以为那件事有人看着 |

⇒ 两条都改成**不认名字**的形状:
* ⑯ 钉活路径 `PendingRow`,并且**数 `!= 200` 的处数 ≥ 2**(批准与拒绝各自都要核返回码);
* ⑰ 钉**开窗这个动作**:轮询那一段里 `ConfirmDialog.Show` / `MessageBox.Show` /
  `.ShowDialog(` 一处都不许有。红测里换成一个**名字完全不同**的弹窗,它照样红。

★ 一条判据钉在**名字**上,那个名字被删掉的那天它就悄悄变成恒真了 ——
而删名字的人看到的是「全绿」。这与本包 §2 那条第七条是同一个形状。

### 将来若要「点某一条才弹确认」

那是**新写一条活路径**,不是复活这 36 行。上面那条不挑函数的判据会自动咬住它
(弹窗里必须出现「逐字」),轮询那条也仍然拦着「由到达时机触发」。
...
* **不声称**删掉那个弹窗改变了任何行为 —— 那条路早就停了,删的是**尸体与寄生在尸体上的断言**;
* **不声称**寄生断言只有那 5 条 —— 只声称:一条**零调用方**的方法上,
  这次真删下去才抖出 5 条(3 条六词 + ⑯ + ⑰),而其中 2 条是**删的时候才发现的**。
  ⇒ 下次遇到死代码,**先删再看谁红**,比先读一遍可靠。
