# `D108` · V14 任务 0:两堵墙的独立量测 —— **实测记录 + 一条待用户重裁的问题**
...
## 3.2 ★★★ 用户澄清(2026-08-08,晚于 §3.1)——【墙②对当前需求**不成立**】

> **用户原意是「不需要以管理员身份运行」,不是「另一个 Windows 账户也能打开管理端」。**

★★ **上面 §2 的量测一个字都不改** —— 它测到的两条(GUI 不能跨会话 · CA 私钥躺在机主的
  用户配置文件里)**结论是对的**,只是**回答的不是用户在问的那个问题**。
  ⇒ 留着。将来若真要做跨账户,那份量测就是现成的起点,不用重新量。

### 两个读法的差别(这才是本节的价值)

| | 读法 —— 我当时量的 | 读法 —— 用户的原意 |
|---|---|---|
| 一句话 | **另一个 Windows 账户**登录后也能打开管理端 | **同一个人**双击就能开,**不用提权** |
| 跨的是什么 | 跨**账户 / 会话** | 跨**完整性等级**(High ⇄ Medium) |
| 今天成不成立 | **不成立**(§2.3、§2.2 两条 OS 性质) | **成立** —— 客户端今天就是这么跑的 |
| 挡它的是什么 | 会话隔离 + 用户配置文件边界 | **没有东西挡** |

### ⇒ 对本轮实现的要求(比原来更简单,但**不是没有要求**)

管理端必须**和客户端一样以普通方式跑**:

1. **HKCU 自启 + `--tray`**,不做服务(§1.3 已经把这条定下来了,与本澄清一致);
2. **不请求提权** —— 清单里不得有 `requireAdministrator`,不靠右键「以管理员身份运行」;
3. ★ 而且要**扛得住被提权启动**:客户端已有 D46 那道护栏
   (`Program.cs:46` 先问"设备密钥打不打得开",打不开就自己降权重开、重开不成才如实拒绝)。
   管理端**同款**,理由一样 —— TPM/CNG 用户密钥绑定**铸造时**的完整性等级。

★ 这一条**不是**"没事可做":它把「不用提权」从一句愿望变成一条**做得到、且验得了**的性质。

### 与 §3.1 那条【甲】的关系

【甲】(管理端只有机主能进)**仍然是本轮的实现口径**,但**理由换了**,这一点要写清楚:

- **原来的理由**(§3.1):跨账户做不到,所以缩范围 —— 那是**受限于 OS**;
- **现在的理由**(本节):用户**本来就没要**跨账户 —— 那是**需求如此**。

★ 差别不是文字游戏:前者意味着"将来有人会来推翻它",后者意味着"这本来就是对的形状"。
  ⇒ 将来第二位家庭成员真要用时,**仍然**要走一次新裁定(§2.4 那条 08-03 的记录不变),
  而那时要解的是**跨账户**那个问题,不是"把本条改回去"。
...
> 取号时仓库里已提交的最大号是 **D107**)。与 `admin-app-lifecycle-2026-08-07.md` **同号**。
