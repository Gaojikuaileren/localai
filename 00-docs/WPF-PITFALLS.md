...
## 4. ★★ `ContentControl` 的默认模板**不画** `Background`
...
## 6. ★ 主题模板里硬编码的 `Background` 会让局部 `Background` 失效
...
## 7. ★ `DoubleAnimation` + `AutoReverse` + `RepeatBehavior(2)` 停在**起始值**
...
## 10. ★ `BringIntoView` 在 `Loaded` 优先级下还没有布局
...
- [ ] 动画结束后停在哪个值,确认过吗?(第 7 条)
- [ ] 悬停/焦点相关的改动,有没有"布局在光标底下重排"?(第 8 条)
