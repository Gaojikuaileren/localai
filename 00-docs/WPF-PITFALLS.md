...
> ★★★ **2026-08-10 补一刀 —— 本条的「每处照办」已被第 15 条取代,别照旧逐处挂。**
> 本条下面写的是「新写的内层滚动/缩放区**一律照办**」(逐处记得挂),
> 而第 15 条(管理端页中页那次)的结论是:**用类处理器一网打尽,没有「记得挂」这一步**。
> 两条规矩并存而本条不指向那条 ⇒ **只读到这里的人会照旧逐处挂,而那正是第 15 条要废掉的做法。**
> ★ 并且本条隐含一个今天不成立的假设:「内层滚动区是你自己写出来的」——
> 页中页那个内层是页面工厂**顺手返回**的,**写的人根本不知道自己造了一个 ScrollViewer**。
> ⇒ **今天的接法见第 15 条。**
新写的内层滚动/缩放区一律照办。
