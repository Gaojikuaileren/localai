...
| | |
|---|---|
| ✅ **A7** 桌面重负载显存上界 | 实测 **6.53 GiB** → 🔄 换方案 · `desktop_floor` **6.6**(`calibrated: partial`)· AI 侧上限 **8.52** |
| ✅ **A1** 每进程 CUDA context | 实测 **227 MiB** → ✅ 接受(阈值 <512)。⚠ 是裸 context 下界,**不能拿去改 §8.1.2** |
| ✅ **E5**(部分)零显存断言 | **0.0 MiB** ✓ D12 成立。脉冲耗时与日累计核心秒仍未测 |
| ⏭ 下一项 | **A2 逐组件 peak + 可加性** —— 表需要的数由它提供 |
| ⛔ 卡住的 4 项 | A2 的 3 行 · A5 整项 · C4 · A8 —— **等语音栈与 VLM 选型** |

**日常常驻组成已定**:`8b@16k` + `speech.lite`(peak **8.2**,余量 0.32)。
