...
**P2 进度续**:✅ **PostgreSQL 18**(SSPI)· ✅ **Qdrant×2**(mem_main/mem_s2 · key 隔离)· ✅ **PG schema**(24 表/2 角色/v_memory_nons2 · 否定用例 20/20 全通过,含行级 S2 实攻)—— 记忆库骨架完成 · 下一步 E1 凭证检测器 + embedding/rerank + 网关 SID 校验。原下一步:PostgreSQL+Qdrant(记忆骨架,P3a 前提)· ✅ **账户隔离**(ai-mem/asset/exec 建成 + memory 目录 Deny ACL,核验通过)· E1 凭证检测器 · secret_ref 建表 · embedding/rerank(CPU)· Open WebUI。
