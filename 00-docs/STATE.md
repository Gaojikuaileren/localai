...
| 事项 | 阻塞了什么 | 需要什么才能解 |
|---|---|---|
| ~~G: 未加密~~ | ~~P3a~~ | ✅ **已解除(D21)**:用户决定介质不加密。`backup.ps1` 已改为按配置放行 |
| **★ 记忆库备份能力未验证** | **P3a 的恢复演练** | 见 §8.5.5。挂载中的 `.vhdx` 可能被 robocopy 静默跳过,而脚本会报告「成功」。**设计中** |
| **C: 未加密** | **D9(记忆卷自动解锁)** | TPM 已实测 Present/Ready/Enabled,方案 A1 可行。**但 D21 之后需确认 D2/D9 是否仍成立 —— 见下** |
| **待确认:D2/D9 是否仍成立** | P3a 记忆卷形态 | D21 说的是**备份介质**不加密。**记忆库本身**是否仍走 BitLocker VHDX(D2)需你确认 |
