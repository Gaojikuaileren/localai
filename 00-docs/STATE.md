...
| # | 事项 | 判据 / 影响 |
|---|---|---|
| 1 | **Tailscale 留不留** | 选 Signal 后 Funnel 用途消失,它只剩"LAN 加密 + 准入 + 设备清单",而那三项可由 mTLS + 管理端替代且**断网时更硬**。**判据只有一个:会不会带笔记本出门用全功能?** ★ **Codex P3b 决策包已并入**(`00-docs/decision-packets/…`),评审中;此项 = 决策包 §12 Q2,待用户回答 |
...
| **P3b** | 待决事项 1 裁定 | ❌ 决策包已并入,评审中;待用户回答 §12 四问 + "可以开始" |
| **P3b** | **LAN TLS 与绑 LAN 捆绑交付** | ❌ 未做。**做不出 TLS 之前继续只绑回环**。决策包定顺序 S0–S7,S1–S4 回环门禁全过前禁止非回环监听 |
