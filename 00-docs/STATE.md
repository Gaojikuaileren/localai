...
| S5 | 记忆面板的**后端 API**(浏览/编辑/删除/溯源)—— ★ **UI 已移至 P3c 客户端**(D36) | ⏭ 下一步 |
| S6 | 冷启动初始化会话 + S2 分级(只跑一次) | |
| S7 | 评测集 + 四指标 + 阈值标定 | |
| S8 | L4 写路径 + 哈希绑定确认 | |
| S9 | 备份/恢复演练 + P3a 验收签字 | |

**四条验收例句均已端到端跑通(每条一句话能说清):**
- S1「我妹妹叫小雨」→ 问名字 → 走**结构化轨**答出,溯源俱全;
- S2「上次聊的那个灯光问题」→ 走**向量轨**,命中金标,**只给 passages 不给 answer**;
- S3「网页上读到的东西,不能自己变成你的记忆」→ web_content **停在待审队列**,确认须带票据;
- S4「我家住哪」→ 地址**进得了库,但出不了门**:标 域S2 · 本机可见 · 局域网/外联/出境后端一个都取不到。

回归口径:**371 断言**(tainted 64 · route 55 · repo 31 · gate 46 · S1 22 · S2 43 ·
S3 冒烟 25 · S3 验收 50 · S4 验收 35)+ 网关侧 123 + `verify.sql` 17 + 隔离套件 16/0/2。

### ★ S3/S4 揪出并修掉的「看着有防护、实际没有」(累计,均实测可证)

| # | 缺陷 | 性质 |
|---|---|---|
| 1 | §4.5 铁律因 `is_user_fact` 要求 sc≥1.0 而保护范围为空 | 我在早前修它时又引入了「低档 supersede 高档」,S3 用**权威序**一并修 |
| 2 | 「强制 pending」无强制点 —— web_content 可静默成为活跃 L3 | S3 分流 |
| 3 | 两套状态列,CHECK 守着没人改的那个 | S3 统一到 status + 终态守卫 |
| 4 | `pr_derived_forced_pending` 把派生候选**永久锁死**(死锁 = DoS) | S3 改 BEFORE INSERT |
| 5 | 拒绝审计与被拒的写同事务 → rollback 一起抹掉 | S3 独立连接 |
| 6 | **域S2 是空集** —— 凭证与机密被合并,S2 分支是死代码 | S4 概念分叉(sensitivity.py) |
| 7 | `TaintedText.__eq__` 是**不记账的猜测-确认预言机** | S4 改比句柄 |
| 8 | `unseal_for_prompt` 判 sensitivity 而非 egress → S0 记忆静默出境 | S4 改 Backend 契约 |
| 9 | **PG 服务器日志**写入正文片段(触发器带值 + 失败语句) | S4 去值 + 钉死 `log_parameter_max_length_on_error=0` |
| 10 | `caller` / `egress` 裸值 = denylist 形状,新档位/新别名默认放行 | S4 枚举 + allowlist + fail-closed |

> 这一族缺陷共出现 **≥5 次**,形状都是**判据写成放行优先,新增的东西默认自由**
> (provenance denylist · E1 override · unseal caller · egress 缺字段 · append-only 新列)。
> 已成为本项目的一条固定审查视角:**任何白/黑名单,先问「加一个新值,它默认落哪边」。**
