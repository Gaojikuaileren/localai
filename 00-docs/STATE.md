...
| 事项 | 阻塞了什么 | 需要什么才能解 |
|---|---|---|
| **G: 未加密**(BitLocker Off) | **P3a** · 且 `backup.ps1` 现在会**拒绝执行** | 你启用 BitLocker To Go。逃生口 `-IAcceptUnencryptedTarget` 会写审计 |
| **C: 未加密 + fTPM 状态未知** | **D9(记忆卷自动解锁)** | `Get-BitLockerVolume` / `Get-Tpm` 核实;然后启用 C: BitLocker(TPM-only) |
| BitLocker 恢复密钥保管方式未定 | P3a 记忆库加密卷 | 离线两份,不同物理位置,**一份在家中可取处**(D9 配套) |
| **命名决定未做** | **P2 的设备身份实现** | 定死 tailnet 名与机器名。RP ID 不可更改 |
