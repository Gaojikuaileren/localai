...
> 回归口径:identity 11+15+22+14(成员层)+13(证书续签)· lan-edge 8+6+15(管理面 E2E)· transport 5 · **客户端 selftest 224**(外壳 30 → 主页打磨与导航/设置改造后)
>
> ★ **前端进度实话**(2026-07-30):**主页阶段性完成**(日历/待办事项/天气/项目/问候/左导航/扩展/设置全部打磨到位,含拖拽排序·竖直转盘·完成划出动画·置顶·板块显隐·三皮肤·三语),但**前端整体还有很多**(六个工作空间仍是明说未做的占位)。**P3c 离收尾还有距离。**
>
> ★ **仍是"外壳 + 诚实占位"**:主页板块的数据源都未接,均如实标注、绝不伪造(设计 §4 / 状态矩阵 §8):
>   日历(未接 Apple 家庭共享日历,保存如实拒绝)· 天气(未接白名单气象端点;"当前地"是时区推断非定位)·
>   待办(增删改内存当场生效,跨设备同步/落盘待中枢)· token 用量(未接,显"待接入")· 问候副句(本地精选,非模型生成,界面不谎称 AI 生成)。
...
| **P3c 统一客户端** | 🔵 **进行中(2026-07-30)** —— S0 选型 ✅(D47 WPF)· S1 成员层 ✅(14/14)· S2 管理面 ✅(15/15,仅回环 D48)· S3 WPF 外壳 ✅· S1b 证书续签 ✅(13/13,D49)· **主页 UI 阶段性完成 ✅**(客户端 selftest 224 全绿:主页四板块 + 左导航「系统组贴底 + 扩展选显示项 + 工作空间/主页板块两类扩展」+ 设备并入设置 + 左下角改「主机状态 + token 用量」+ 财务/投资工作空间)。**剩(离收尾还有距离):** S4 配对界面接回环管理面【跑实机】· S5 显存释放接 gpu-broker(P4 内核)· 六个工作空间的实际功能(依赖 P4/P6/P9)· 主页各板块接真实数据源 · 桌面宠物(用户指定回来再做)· **主页数据「来源 × AI 可读/可操作 × 回滚」矩阵**(用户提出,待设计定稿,见待决) |
