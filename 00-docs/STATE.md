...
| **P3a 记忆系统** | ⏭ **可以开始** —— 硬前置(活库备份方式)2026-07-28 已解除 |
...
| **★ 局域网直连的命名 + 证书方案未定** | **P3b 的局域网路径**(D32) | ★ 与 RP ID 同一个陷阱:WebAuthn RP ID 一经创建凭证即不可改,局域网路径需**自己的** RP ID(与 tailnet 不同 origin)。且 WebAuthn **要求 HTTPS**(浏览器只对 localhost 例外)→ `http://192.168.x.x` 用不了 passkey,需手机信任的证书(本地 CA / 真域名+本地解析)。另:LAN IP 是 DHCP 分的,需地址保留或固定名。**应作为 P3b 第一项决定,如同「命名决定」之于 P2** |
